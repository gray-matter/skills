...
Codebase Walkthrough is a skill bundle for generating concise onboarding walkthrough packs under `docs/walkthrough/` in target repositories. It also includes a small Python-hosted Docsify viewer for reading those artifacts with clickable source references.
...
- A modern browser with access to jsDelivr when using the viewer, because `viewer/index.html` loads Docsify from the CDN.
...
|   `-- serve_viewer.py               # Python stdlib server for rendering target repo walkthrough artifacts
...
`-- viewer/                           # Static Docsify-based walkthrough reader assets
    |-- app.js                        # Panel rendering, source-link handling, and prompt-copy behavior
